...
/* Subroutine */ int s3key_(char *key, integer *loc, ftnlen key_len)
{
    ...
    static char keys[16*90] = "AIJ             " "BACKUP          " "BOUNDS "
	    "         " "CALL            " "CENTRAL         " "CG            "
	    ...
	    "TIME            " "TIMING          " "TOTAL           " "UNBOUN"
	    "DED       " "UPPER           " "USER            " "VERIFY       "
	    "   " "VIOLATION       " "WARM            " "WORKING         " 
	    "WORKSPACE       ";
...
/*     ------------------------------------------------------------------ */
/*     ------------------------------------------------------------------ */
    oplook_(&c__90, keys, &c_true, key, loc, (ftnlen)16, (ftnlen)16);
    return 0;
} /* s3key_ */
...
/*              Grab the contents of  iw(i) */
		...
		*r__ = (doublereal) (*i__);
	    ...
	    if (s_cmp(key2, "FACTOR      ", (ftnlen)16, (ftnlen)12) == 0) {
		s3optr_(s, &rw[66], r__);
	    }
	    if (s_cmp(key2, "ROOK        ", (ftnlen)16, (ftnlen)12) == 0) {
		s3optl_(s, &iw[80], &i1, i__);
	    }
	    if (s_cmp(key2, "UPDATE      ", (ftnlen)16, (ftnlen)12) == 0) {
		s3optr_(s, &rw[67], r__);
	    }
...
/*              Grab the contents of  rw(i) */
	...
	if (s_cmp(key, "STICKY      ", key_len, (ftnlen)12) == 0) {
	    ...
	    if (s_cmp(key3, "YES      ", (ftnlen)16, (ftnlen)9) == 0) {
		s3optl_(s, &iw[116], &i1, i__);
	    }
	    if (s_cmp(key3, "NO       ", (ftnlen)16, (ftnlen)9) == 0) {
	    ...
	    s3opti_(s, &iw[81], i__);
	} else if (s_cmp(key, "TIME        ", key_len, (ftnlen)12) == 0) {
	    s3optr_(s, &rw[79], r__);
...
/* Subroutine */ int s3tie_(char *tie, integer *loc, ftnlen tie_len)
{
    ...
    static char ties[16*72] = "(TOTAL)         " "(USER)          " "ALL    "
	    "         " "BASIC           " "BASIS           " "BOUND         "
	    ...
	    "          " "FACTORIZATION   " "FEASIBILITY     " "FILE         "
	    "   " "FLUSH           " "FREQUENCY       " "FULL            " 
	    "GRADIENTS       " "INFORMATION     " "INTEGER         " "ITERAT"
	    "IONS      " "JACOBIAN        " "LEVEL           " "LIMITED      "
	    "   " "LINEAR          " "LINESEARCH      " "LOG             " 
	    "MODE            " "NEWTON          " "NO              " "NONLIN"
	    "EAR       " "OBJECTIVE       " "OPTIMALITY      " "OPTION       "
	    "   " "PARAMETERS      " "PARTIAL         " "PHASE1          " 
	    "PHASE2          " "POINT           " "PRECONDITIONING " "PRINT "
	    "          " "QN              " "REAL            " "ROOK         "
	    "   " "ROW             " "SINGULARITY     " "SOLVER          " 
	    "SPARSE          " "START           " "STEP            " "STOP  "
	    "          " "SUPERBASICS     " "SWAP            " "TOLERANCE    "
	    "   " "UPDATE          " "UPDATES         " "VARIABLES       " 
	    "WEIGHT          " "WEIGHTMAX       " "YES             ";
...
/*     ------------------------------------------------------------------ */
/*     ------------------------------------------------------------------ */
    oplook_(&c__72, ties, &c_true, tie, loc, (ftnlen)16, (ftnlen)16);
    return 0;
} /* s3tie_ */
