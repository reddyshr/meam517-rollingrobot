/* ../snopt7/src/snoptq.f -- translated by f2c (version 20100827).
...
/*     eType(n+m) indicate if the variable can violate its bound in */
/*             Elastic mode. */
/*             if eType(j) = 0, variable j is non-elastic and must not */
/*                               be violated. */
/*             if eType(j) = 1, variable j can violate its lower bound. */
/*             if eType(j) = 2, variable j can violate its upper bound. */
/*             if eType(j) = 3, variable j can violate either its */
...
/*     ------------------------------------------------------------------ */
    ...
    /* Function Body */
    snkerq_(start, (U_fp)qphx, (U_fp)sqlog_, m, n, nea, nnames, lencobj, 
	    ncolh, iobj, objadd, prob, &acol[1], &inda[1], &loca[1], &bl[1], &
	    bu[1], &cobj[1], names + 8, &etype[1], &hs[1], &x[1], &pi[1], &rc[
	    1], info, mincw, miniw, minrw, ns, ninf, sinf, obj, cu + 8, lencu,
	     &iu[1], leniu, &ru[1], lenru, cw + 8, lencw, &iw[1], leniw, &rw[
	    1], lenrw, start_len, (ftnlen)8, (ftnlen)8, (ftnlen)8, (ftnlen)8);
    return 0;
} /* snoptq_ */
...
/* Subroutine */ int snkerq_(char *start, U_fp qphx, U_fp sqlog, integer *m, 
	integer *n, integer *nea, integer *nnames, integer *lencobj, integer *
	...
	ftnlen cw_len)
{
    ...
    static integer lenx0, nrhs0, nnjac, nloca, ninfe, ngobj, nloch;
    ...
    extern /* Subroutine */ int s1file_(integer *, integer *, integer *), 
	    ...
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *), s1time_(integer *, integer *, integer *, integer *, 
	    doublereal *, integer *), s1perm_(integer *, integer *), chcopy_(
    ...
    extern /* Subroutine */ int s5solve_(integer *, char *, integer *, U_fp, 
...
/*     Check the input arguments. */
    starttype = iw[69];
    s3chkargsq_(&inform__, start, m, n, nea, nnames, ns, lencobj, iobj, ncolh,
	     &inda[1], &loca[1], &bl[1], &bu[1], names + 8, &hs[1], &pi[1], &
	    starttype, &errors, &iw[1], leniw, &rw[1], lenrw, start_len, (
	    ftnlen)8);
    if (inform__ > 0) {
	*info = inform__;
	goto L800;
...
/*     ------------------------------------------------------------------ */
    ...
    s5defaults_(m, n, lencobj, ncolh, cw + 8, lencw, &iw[1], leniw, &rw[1], 
	    lenrw, (ftnlen)8);
    s3printq_(m, n, lencobj, ncolh, &starttype, &iw[1], leniw, &rw[1], lenrw);
    maxr = iw[52];
/* max columns of R. */
    maxs = iw[53];
/* max # of superbasics */
...
/*     ------------------------------------------------------------------ */
    nloca = *n + 1;
    s5solve_(info, solver, &starttype, (U_fp)sqlog, (U_fp)sqhx_, (U_fp)qphx, &
	    gotr, m, n, &nb, &nnh0, &nnh, nnames, &ngqp, &ngobj0, &ngobj, 
	    iobj, objadd, &objqp, &objtrue, ninf, sinf, &ninfe, &sinfe, nea, &
	    nloca, &loca[1], &inda[1], &acol[1], &neh, &nloch, loch, indh, 
	    hcol, &bl[1], &bu[1], &cobj[1], names + 8, &nrhs0, &nrhs, rhs, &
	    lenx0, &nx0, x0, &etype[1], &hs[1], &x[1], &pi[1], &rc[1], ns, cu 
	    + 8, lencu, &iu[1], leniu, &ru[1], lenru, cw + 8, lencw, &iw[1], 
	    leniw, &rw[1], lenrw, (ftnlen)6, (ftnlen)8, (ftnlen)8, (ftnlen)8);
    *ninf += ninfe;
    *sinf += sinfe;
    *obj = objqp;
    *mincw = iw[47];
