...
!     27 Apr 2015: Added SNCTRL options.
!     ==================================================================
      integer                 maxkey
      parameter         (     maxkey = 94)
...
!     ==================================================================
     ...
     &     kDegen, kfac, klog, kReset, ksav, kSumm, lDenJ, eMode,
     &     lprDbg, lprPrm, lprSch, lprScl, lprSol, lvlDer, lvlHes,
     &     lvlObjE, lvlPiv, lvlPre, lvlPPm, lvlSch, lvlScale, lvlStart,
     ...
     &     nnL, nParPrU, nProb, npStatus, ObjRow, QPslvr, qpStatus,
     &     s1intmx, stickyOp
      integer
     &     etarg, tolCG, tolOptFP, tolOptQP, tolOptNP, tolx, tolCon,
     &     tolpiv, tCrash, tolswp, tolfac, tolupd, infBnd, bigFx,
     &     bigdx, epsrf, fdint1, fdint2, xdlim, vilim, wolfeG, wtInf0,
     &     mNewSB, xPen0, scltol, Aijtol, bStrc1, bStrc2, Utol1, Utol2,
     &     Dens2, wtMax, maxTime
      integer ! snctrl
     &     ctRefTol, iDisc, iCPrt, iRefn, iRefL
!     ------------------------------------------------------------------
      integer                  maxtok
      parameter         (      maxtok = 10)
      ...
      parameter         (tolOptFP  =  51) ! Minor Phase 1 Opt tol
      parameter         (tolOptQP  =  52) ! Minor Phase 2 Opt tol
      parameter         (tolOptNP  =  53) ! Major Optimality tolerance
      ...
      parameter         (maxTime   =  79) ! time limit
      ...
      parameter         (eMode     =  56) ! >0    => use elastic mode
      ...
      parameter         (lvlObjE   =  73) ! Elastic objective type
      ...
      parameter         (nParPrU   = 101) ! # partial pricing sections (user-def)
      ...
      parameter         (stickyOp  = 116) ! > 0 => sticky parameters
      ...
      parameter         (ctRefTol  = 441) ! snctrl refinement tol
      parameter         (iDisc     = 442) ! snctrl
      parameter         (iCPrt     = 443) ! snctrl
      parameter         (iRefn     = 444) ! snctrl
      parameter         (iRefL     = 445) ! snctrl
...
!     Keywords for SNCTRL.
      if (more) then
         more   = .false.
         if      (key  .eq. 'DISCRETIZATION') then
            if  (loc2 .eq.  0           ) go to 820
            if  (key2 .eq. 'HS          ') call s3optl(s,iw(iDisc),i1,i)
            if  (key2 .eq. 'TR          ') call s3optl(s,iw(iDisc),i0,i)
         else if (key  .eq. 'REFINEMENT    ') then
            if  (loc2 .eq.  0           ) go to 820
            if  (key2 .eq. 'NO          ') call s3optl(s,iw(iRefn),i0,i)
            if  (key2 .eq. 'YES         ') call s3optl(s,iw(iRefn),i1,i)
            if  (key2 .eq. 'TOLERANCE   ') call s3optr(s,rw(ctrefTol),r)
            if  (key2 .eq. 'LIMIT       ') call s3opti(s,iw(irefL),i)
         else if (key  .eq. 'CONTROL       ') then
            if  (loc2 .eq.  0           ) go to 820
            if  (key2 .eq. 'SOLUTION    ') then
               if(key3 .eq. 'NO          ')call s3optl(s,iw(iCPrt),i0,i)
               if(key3 .eq. 'YES         ')call s3optl(s,iw(iCPrt),i1,i)
            end if
         else
            more   = .true.
         end if
...
!     Keywords for SNOPT.
      if (more) then
         ...
         if      (key .eq. 'BACKUP      ') then
            ...
            call s3opti(s, iw(qpStatus), i)
            call s3opti(s, iw(npStatus), i)
     ...
     &         ( cw, lencw, iw, leniw, rw, lenrw )
            ...
            call s3opti(s, iw(iDump ), i)
              ...
              if (loc2.eq.  0            ) go to 820
              if (key2.eq. 'OBJECTIVE   ') call s3opti(s, iw(lvlObjE),i)
              if (key2.eq. 'MODE        ') call s3opti(s, iw(eMode ), i)
...
!              Grab the contents of  iw(i)
            ...
            call s3opti(s, iw(klog)  , i)
              ...
              if (key2.eq. 'OPTIMALITY  ') call s3optr(s,rw(tolOptQP),r)
              ...
              if (key2.eq. 'FACTOR      ')call s3optr(s, rw(tolFac), r)
              if (key2.eq. 'ROOK        ')call s3optl(s,iw(lvlPiv),i1,i)
              if (key2.eq. 'UPDATE      ') call s3optr(s, rw(tolUpd), r)
...
!              end if
      ...
      if (more) then
         ...
         if      (key .eq. 'MAJOR       ') then
              ...
              if (key2.eq. 'OPTIMALITY  ') call s3optr(s,rw(tolOptNP),r)
            ...
            call s3opti(s, iw(minmax), i1)
              ...
              if (key2.eq. 'FEASIBILITY ') call s3optr(s,rw(tolx  )  ,r)
              if (key2.eq. 'OPTIMALITY  ') call s3optr(s,rw(tolOptQP),r)
              if (key2.eq. 'PHASE1      ') call s3optr(s,rw(tolOptFP),r)
              if (key2.eq. 'PHASE2      ') call s3optr(s,rw(tolOptQP),r)
     ...
     &            key .eq. 'NON         ') then
            ...
            call s3optr(s, rw(tolOptQP), r)
         else if (key .eq. 'PARTIAL     ') then
            call s3opti(s, iw(nParPrU), i)
...
!              Grab the contents of  rw(i)
            ...
            call s3opti(s, iw(ksav  ), i)
              ...
              if (loc2.eq.  0            ) go to 820
              if (key2.eq. 'OPTION      ') call s3opti(s,iw(lvlScale),i)
      ...
      if (more) then
         more   = .false.
         if      (key .eq. 'STICKY      ') then
              if (loc2.eq.  0            ) go to 820
              if (key3.eq. 'YES      ') call s3optl(s,iw(stickyOp),i1,i)
              if (key3.eq. 'NO       ') call s3optl(s,iw(stickyOp),i0,i)

         else if      (key .eq. 'SUBSPACE    ') then
...
!     07 Jan 2015: Added tie for 'LU factor tolerance': 'Factor'
!     07 Jan 2015: Added tie for 'LU update tolerance': 'Update'
!     27 Apr 2015: Added SNCTRL options.
!     ==================================================================
      integer                 maxtie
      parameter         (     maxtie = 75)
