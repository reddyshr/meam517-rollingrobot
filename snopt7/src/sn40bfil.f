...
!        Find the maximum bound and dual infeasibilities.
     ...
     &      ( n, nb, iObj, eps0, bl, bu, rc, x, dInf, jdInf )
         ...
         ! 17 Aug 2015: If iReport = 94, 95 or 96,
         !              export A, B or (B S) to Report file 91, 92 or 93
         !              This happens BEFORE UNSCALING, so we can get any
         !              scaled data that may have caused trouble in LUSOL.
         !              It preempts 91, 92 or 93.

         if (iReport .ge. 94  .and.  iReport .le. 96) then
            iReport = iReport - 3
            call s2xmat
     &         ( iReport, n, nb,
...
!     08 Feb 2016  ROWS/COLUMNS sections now CONSTRAINTS/VARIABLES.
!     ==================================================================
      character
     &     form1*4, form2*4,
...
!!    scaled    = lvlScale .gt. 0
     ...
     &       ' Section 1 - Constraints'
         call snPRNT( 11, str, iw, leniw )
         write(str, '(a)')
     &       '  Number  ...Row.. State  ...Value......  Slack Value...'
     &     //'  ..Lower Limit.  ..Upper Limit.  .Dual Variable    ..i'
         call snPRNT( 11, str, iw, leniw )
...
!     ------------------------------------------------------------------
     ...
     &       ' Section 2 - Variables'
         call snPRNT( 1, str, iw, leniw )
         write(str, '(a)')
     &       '  Number  .Column. State  ...Value......  .Obj Gradient.'
     &     //'  ..Lower Limit.  ..Upper Limit.  .Dual Variable    m+j'
         call snPRNT( 11, str, iw, leniw )
         call snPRNT(  1, ' ', iw, leniw )
      end if
 ...
 1008 format(
     &   ' Bounds',    9x, a8)
 1010 format(/ ' Section 1 - Constraints' //
     &   '  Number  ...Row.. State  ...Value......  Slack Value...',
     &   '  ..Lower Limit.  ..Upper Limit.  .Dual Variable    ..i' /)
 1020 format(' Section 2 - Variables' //
     &       '  Number  .Column. State  ...Value......  .Obj Gradient.',
     &       '  ..Lower Limit.  ..Upper Limit.  Dual Variable.    m+j'/)
