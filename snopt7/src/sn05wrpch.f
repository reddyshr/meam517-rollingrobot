...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     NonlinearCon, NonlinearObj
      integer
     &     iExit, lencu, leniu, lenru, lencw, leniw, lenrw, funMode, n,
...
!     their gradients.
!
!     Argument  userfun  is called using funMode to control
...
!     ------------------------------------------------------------------
      ...
      if (Scaled) then
         ...
         if (NonlinearCon) then
            if (funMode .gt. 0  .and.  iw(gotGl) .gt. 0) then
      ...
      !=================================================================
      mode       = funMode      ! In case userfun alters funMode
      ...
      if (FPonly) then
         yObj = zero
         call userfun
     &      ( mode, nnObjU, nnCon, nnJac, nnH, negCon, neH,
     &        x, yObj, yCon,
     &        fObj, rw(lgSave), fCon, gCon, Hcol, statusUser,
     &        cu, lencu, iu, leniu, ru, lenru )
     ...
     &        cu, lencu, iu, leniu, ru, lenru )
      ...
      if (Scaled) then
         ...
         if (NonlinearCon  .and.  mode .ge. 0) then
            ...
            if (funMode .gt. 0  .and.  iw(gotG) .gt. 0) then
               call s8scaleJ
