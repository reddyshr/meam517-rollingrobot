...
! 07 May 2016: lu1fad sets nrank = n - nsing after lu1ful.
!***********************************************************************
!
!     File lusol1.f
!
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                   Lmax  , Umax  , DUmax , DUmin , Akmax )
      ...
      double precision   abest, Lij, Ltol, small
  ...
  300    ncold  = lenr(ibest)
         ...
         if (lprint .ge. 50) then
            ...
            if ( TPP .or. TRP .or. TSP ) then
               write(nout, 1200) nrowu, ibest, jbest, abest,
     &                           nrowd, ncold
            else ! TCP
               jmax   = Hj(1)
               imax   = indc(locc(jmax))
               write(nout, 1200) nrowu, ibest, jbest, abest,
     &                           nrowd, ncold, imax , jmax , aijmax
            end if
         end if
...
!        call timer ( 'start ', 17 )
     ...
     &                lenc  , lenr , locc , ipinv, locr )
         ...
         ! 07 May 2016: Still trouble with singular examples from
         !              Jennifer Scott (m>n).  Set nrank = n - nsing after all.
 ...
 1100 format(/ 1x, a)
 1200 format(' nrowu', i7, '   i,jbest', 2i7, '   abest', es10.2,
     &     '   nrowd,ncold', 2i6, '   i,jmax', 2i7, '   aijmax', es10.2)
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                   lenc(n)   , lenr(m)   , iploc(n), iqloc(m)
      integer            locc(n)   , locr(m)
      double precision   abest
      integer            ibest, jbest, mbest
      ...
      ! 27 Apr 2016: abest is another output for printing in lu1fad.
      !
      ! Systems Optimization Laboratory, Stanford University.
      !-----------------------------------------------------------------

      double precision   aij, amax, cmax, lbest
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                   merit, ncol, nrow, nz, nz1
      double precision   aij, amax, atoli, atolj
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                   lq, lq1, lq2, merit, ncol, nz, nz1
      double precision   aij, amax, atolj
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                   indr(lena), lenr(m), locr(m), p(k2)
      ...
      else                      ! Later calls: Find Amaxr(i) for i = p(k1:k2).
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                   lena, a, ind, len, loc )
      ...
      !              ind(*) and/or a(*) arrays and given one empty space.
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  200 continue
     ...
     &                ipvt, iq(nrowu) )
      ...
      ! lkk points to D(k,k), the kth diagonal of U in D.
      ! lkn points to D(k,nleft), the last element in the kth row of U in D.
      !-----------------------------------------------------------------
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                   ipvt, iq )
      ...
      ! 01 May 2015: Optional debug output.
      !-----------------------------------------------------------------
      !
      ! On entry:
      !
      ...
      if (abs( a(l,k) ) .le. small) then
       ...
       ! end if
         ...
         if (k .lt. m) then
          ...
          ! if (debug) then     !!! DEBUG
          !    write(*,*) 'lu1DPP k =', k, '   amax =', a(k,k) ! diag
          ! end if
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  200    ipvt(k) = imax
         ...
         if (aijmax .le. small) then
          ! if (debug) then
          !    write(*,*) 'lu1DCP k =', k, '   aijmax is small:', aijmax
          ! end if
            go to 500    ! break
         end if
         ...
         if (k .lt. m) then
          ...
          ! if (debug) then     !!! DEBUG
          !    write(*,*) 'lu1DCP k =', k, '   amax =', t !, aijmax
          ! end if
