...
subroutine sqmxSolve(nlhs, plhs, nrhs, prhs)
  ...
  if (freebie) then
     if (n > maxCol) call mexErrMsgIdAndTxt('SQOPT:InputArg','Maximum column size of 300 exceeded')
     if (m > maxRow) call mexErrMsgIdAndTxt('SQOPT:InputArg','Maximum row size of 300 exceeded')
  end if
