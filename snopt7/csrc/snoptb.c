/* ../snopt7/src/snoptb.f -- translated by f2c (version 20100827).
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* Subroutine */ int snopt_(char *start, integer *m, integer *n, integer *nej,
	 integer *nnames, integer *nncon, integer *nnobju, integer *nnjac, 
	integer *iobju, doublereal *objuadd, char *prob, U_fp funcon, U_fp 
...
/*     ------------------------------------------------------------------ */
    ...
    /* Function Body */
    snkerb_(start, m, n, nej, nnames, nncon, nnobju, nnjac, iobju, objuadd, 
	    prob, (U_fp)funcon, (U_fp)funobj, (U_fp)snlog_, (U_fp)snlog2_, (
	    U_fp)sqlog_, (U_fp)snstop_, &jcol[1], &indj[1], &locj[1], &bl[1], 
	    &bu[1], names + 8, &hs[1], &x[1], &pi[1], &rc[1], info, mincw, 
	    miniw, minrw, ns, ninf, sinf, obj, cu + 8, lencu, &iu[1], leniu, &
	    ru[1], lenru, cw + 8, lencw, &iw[1], leniw, &rw[1], lenrw, 
	    start_len, (ftnlen)8, (ftnlen)8, (ftnlen)8, (ftnlen)8);
    return 0;
} /* snopt_ */

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine snOpt */
/* Subroutine */ int snoptb_(char *start, integer *m, integer *n, integer *
	nej, integer *nnames, integer *nncon, integer *nnobju, integer *nnjac,
	 integer *iobju, doublereal *objuadd, char *prob, U_fp funcon, U_fp 
	funobj, doublereal *jcol, integer *indj, integer *locj, doublereal *
	bl, doublereal *bu, char *names, integer *hs, doublereal *x, 
	doublereal *pi, doublereal *rc, integer *info, integer *mincw, 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine snOptB */
/* Subroutine */ int snkerb_(char *start, integer *m, integer *n, integer *
	nej, integer *nnames, integer *nncon, integer *nnobju, integer *nnjac,
	 integer *iobju, doublereal *objuadd, char *prob, U_fp funcon, U_fp 
	...
	ftnlen names_len, ftnlen cu_len, ftnlen cw_len)
{
    ...
    static integer stickyop;
    static doublereal x0[1];
    extern /* Subroutine */ int s3chkargsb_(integer *, char *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, doublereal *, 
	    doublereal *, char *, integer *, doublereal *, integer *, integer 
    ...
    static integer lx0, nx0, neh, nnh;
    ...
    extern /* Subroutine */ int iload_(integer *, integer *, integer *, 
	    integer *);
    static integer lgobj, ninfe, llocg, nlocg, nloch, nlocj, nnobj;
    ...
    extern /* Subroutine */ int s8gloc_(integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *),
	     ...
	     integer *), s1perm_(integer *, integer *);
    ...
    static integer nmajor, minmax, inform__, mqnmod, lvlhes, letype;
    ...
    extern /* Subroutine */ int s8gsize_(integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *), s5solve_(
	    integer *, char *, integer *, U_fp, U_fp, U_fp, logical *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, doublereal *, 
	    ...
	    doublereal *, integer *, char *, integer *, integer *, integer *, 
	    doublereal *, integer *, ftnlen, ftnlen, ftnlen, ftnlen), 
	    s8solve_(integer *, char *, integer *, U_fp, U_fp, U_fp, U_fp, 
...
/*     Check the input arguments. */
    starttype = iw[69];
    s3chkargsb_(&inform__, start, m, n, nej, nnames, ns, nncon, nnobju, nnjac,
	     iobju, &indj[1], &locj[1], &bl[1], &bu[1], names + 8, &hs[1], &
	    pi[1], &starttype, &errors, &iw[1], leniw, &rw[1], lenrw, 
	    start_len, (ftnlen)8);
    if (inform__ > 0) {
	*info = inform__;
	goto L800;
...
/*     ------------------------------------------------------------------ */
    ...
    s3printb_(m, n, nncon, nnjac, nnobju, &starttype, &iw[1], leniw, &rw[1], 
	    lenrw);
/*     ------------------------------------------------------------------ */
...
/*     ------------------------------------------------------------------ */
    ...
    nlocj = *n + 1;
    nkx = nb;
    s8gsize_(m, nncon, nnjac, nej, &nlocj, &locj[1], &indj[1], &negcon);
    maxr = iw[52];
/* max columns of R. */
    maxs = iw[53];
/* max # of superbasics */
...
/* locG(nlocG) = column pointers for indG */
	nlocg = *nnjac + 1;
	s8gloc_(nncon, nnjac, nej, &nlocj, &locj[1], &indj[1], &negcon, &
		nlocg, &iw[llocg]);
    }
/*     ------------------------------------------------------------------ */
/*     Solve the problem. */
...
/* gObj(ngObj) = Objective gradient */
	...
	s5solve_(info, solver, &starttype, (U_fp)sqlog, (U_fp)s8hxqp_, (U_fp)
		...
		ninfe, &sinfe, nej, &nlocj, &locj[1], &indj[1], &jcol[1], &
		neh, &nloch, loch, indh, hcol, &bl[1], &bu[1], &rw[lgobj], 
		names + 8, &nrhs0, &nrhs, rhs, &lenx0, &nx0, x0, &iw[letype], 
...
/*        Define the type of initial Hessian. */
	...
	s8solve_(info, solver, &starttype, (U_fp)s0fgb_, (U_fp)funcon, (U_fp)
		funobj, (U_fp)s8hxnp_, (U_fp)snlog, (U_fp)snlog2, (U_fp)
		snstop, &gotr, m, n, &nb, &nnh, nncon, nnjac, &nnobj, nnames, 
		&iobj, &objadd, &fobj, &objtrue, ninf, sinf, &ninfe, &sinfe, 
		nej, &nlocj, &locj[1], &indj[1], &jcol[1], &neh, &nloch, loch,
		 indh, hcol, &bl[1], &bu[1], names + 8, &hs[1], &x[1], &pi[1],
		 &rc[1], &nmajor, ns, cu + 8, lencu, &iu[1], leniu, &ru[1], 
