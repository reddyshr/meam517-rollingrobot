/* ../snopt7/src/sn35mps.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int mpsinp_(integer *imps, integer *maxm, integer *maxn, 
	integer *maxnea, integer *nncon, integer *nnjac, integer *nnobj, 
	integer *m, integer *n, integer *nea, integer *iobj, doublereal *
...
/*     ------------------------------------------------------------------ */
	...
	*iexit = 83;
    ...
    s3mps_(iexit, key, maxm, maxn, maxnea, &lenh, &nnl, nncon, nnjac, m, n, 
	    nea, &negcon, ns, iobj, objadd, &loca[1], &inda[1], &acol[1], &bl[
	    1], &bu[1], names + 8, &hs[1], &iw[lhrtyp], &iw[lkbs], &iw[lkeynm]
...
/* Subroutine */ int mpsout_(integer *imps, integer *m, integer *n, integer *
	nea, integer *nname, char *prbnms, doublereal *acol, integer *inda, 
...
/*     The true values of m, n and neA are also found. */
    lacol = iw[256];
/* Acol(neA)   = Constraints by columns */
    linda = iw[258];
/* indA(neA) holds the row indices for Aij */
    lloca = iw[257];
/* locA(n+1)   = column pointers for indA */
    lbl = iw[271];
/* bl(nb)      = lower bounds */
    lbu = iw[272];
/* bu(nb)      = upper bounds */
...
/* keynm(lenh) = hash table keys */
    s3mps_(iexit, key, maxm, maxn, maxne, &lenh, &nnl, nncon, nnjac, m, n, 
	    nea, &iw[20], &ns, iobj, objadd, &iw[lloca], &iw[linda], &rw[
	    lacol], &rw[lbl], &rw[lbu], cw + (lnames << 3), &iw[lhs], &iw[
	    lhrtyp], &iw[lkbs], &iw[lkeynm], &rw[lx], &rw[lpi], cw + 8, lencw,
	     &iw[1], leniw, &rw[1], lenrw, (ftnlen)4, (ftnlen)8, (ftnlen)8);
    if (*iexit != 0) {
	goto L700;
    }
...
/*     Save current positions of  bl, bu, etc. */
    ...
    *nextiw = newiw;
    *nextrw = newrw;
    s3mapm_(iexit, m, n, nea, &lenh, nextcw, nextiw, nextrw, mincw, miniw, 
...
/* Subroutine */ int s3mapm_(integer *iexit, integer *m, integer *n, integer *
	nea, integer *lenh, integer *nextcw, integer *nextiw, integer *nextrw,
	 integer *mincw, integer *miniw, integer *minrw, integer *lencw, 
	integer *leniw, integer *lenrw, integer *iw)
{
    static integer lfeastype, nb, lx, lbl, lrc, lbu, lpi, lhs, lkbs, linda, 
	    lloca, lacol, lnames, lkeynm, letype;
...
/* Subroutine */ int s3mps_(integer *iexit, char *key, integer *maxm, integer 
	*maxn, integer *maxne, integer *lenh, integer *nnl, integer *nncon, 
	integer *nnjac, integer *m, integer *n, integer *nea, integer *negcon,
	 integer *ns, integer *iobj, doublereal *objadd, integer *loca, 
	integer *inda, doublereal *acol, doublereal *bl, doublereal *bu, char 
	*names, integer *hs, integer *rowtypes, integer *kbs, integer *keynam,
...
/*     ------------------------------------------------------------------ */
    ...
    s3mpsa_(iexit, ier, &line, maxm, maxn, iobj, &ncoll, m, &lrow, &lennam, 
	    lenh, nnl, nncon, key, ncard, &rowtypes[1], &keynam[1], names + 8,
...
/*     ------------------------------------------------------------------ */
    s3mpsb_(iexit, ier, &line, maxn, maxne, &lrow, &lennam, lenh, &ncoll, 
	    iobj, objadd, m, n, nea, nnl, nncon, nnjac, negcon, &na0, key, 
	    ncard, &rowtypes[1], &keynam[1], names + 8, &loca[1], &inda[1], &
...
/* Subroutine */ int s3mpsa_(integer *iexit, integer *ier, integer *line, 
	...
	integer *nncon, char *key, integer *ncard, integer *rowtypes, integer 
...
/* Subroutine */ int s3mpsb_(integer *iexit, integer *ier, integer *line, 
	integer *maxn, integer *maxnea, integer *lrow, integer *lennm, 
	integer *lenh, integer *ncoll, integer *iobj, doublereal *objadd, 
	integer *m, integer *n, integer *nea, integer *nnl, integer *nncon, 
	integer *nnjac, integer *negcon, integer *na0, char *key, integer *
	ncard, integer *rowtypes, integer *keynam, char *names, integer *loca,
	 integer *inda, doublereal *acol, doublereal *bl, doublereal *bu, 
	integer *kbs, doublereal *pi, char *cw, integer *lencw, integer *iw, 
	integer *leniw, doublereal *rw, integer *lenrw, ftnlen key_len, 
	ftnlen names_len, ftnlen cw_len)
{
    /* Initialized data */
...
/*     Or too many columns or elements? */
	...
	*iexit = 112;
	...
	s_wsfi(&io___191);
	do_fio(&c__1, (char *)&(*maxnea), (ftnlen)sizeof(integer));
