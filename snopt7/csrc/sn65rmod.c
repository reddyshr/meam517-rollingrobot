...
/*     s6Radd    s6RBFGS    s6chol   s6mchl   s6pchl   s6Rcnd   s6Rcol */
/*     s6Rdel    s6Rfix     s6Rmod   s6Rprod  s6Rrow   s6Rset   s6Rsol */
/*     s6Rswap   s6Rupdate  s6Rqn */

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* Subroutine */ int s6radd_(integer *maxr, integer *lenr, integer *ns, 
	doublereal *rlast, doublereal *r__)
{
...
/* 11 Dec 2014: Last diagonal element of R  added as argument. */
/* ================================================================= */
/* ----------------------------------------------------------------- */
/* ----------------------------------------------------------------- */
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s6Radd */
/* Subroutine */ int s6rbfgs_(integer *iexit, integer *maxr, integer *ns, 
...
/* ================================================================= */
/* s6RBFGS applies the quasi-Newton update to the first nS rows and */
...
/* 17 Nov 2001: Current version of s6RBFGS. */
/* ================================================================= */
/* ----------------------------------------------------------------- */
/* ----------------------------------------------------------------- */
    /* Parameter adjustments */
...
/* Computing 2nd power */
	    d__2 = *drmin;
	    d__1 = d__2 * d__2;
	    *condh = ddiv_(&c_b13, &d__1, &overflow);
	    if (*condh < 1.) {
		*condh = 1. / *condh;
	    }
	} else {
...
/* Computing 2nd power */
	    d__4 = *drmin;
	    d__3 = d__4 * d__4;
	    *condh = ddiv_(&d__1, &d__3, &overflow);
	}
    }
    return 0;
} /* s6rcnd_ */
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s6Rcnd */
/* Subroutine */ int s6rcol_(integer *job, integer *jr, integer *maxr, 
	integer *nr, integer *lenr, doublereal *r__, doublereal *v, integer *
	lrdiag)
{
    /* System generated locals */
    integer i__1;
...
/* ================================================================= */
/* s6Rcol  either inserts or extracts a column of the upper- */
/* triangular matrix R. */

/* On entry: */
/*   If job = PUSH (=0),  v(1:jR) becomes column jR. */
/*   If job = PULL (=1),  column jR becomes v(1:jR). */

/* lRdiag   returns the location of the new diagonal element. */

/* 03 Dec 2000: s6Radd derived from MINOS routine m6radd. */
/* 06 Dec 2000: s6Radd changed to s6Rcol because we need s6Rrow. */
/* 03 Nov 2014: Added option to extract a column. */
/* ================================================================= */
/* ----------------------------------------------------------------- */
/* ----------------------------------------------------------------- */
    /* Parameter adjustments */
...
/* dRmax, dRmin, Hcondbnd are input. */
...
/* ----------------------------------------------------------------- */
    ...
    nr = min(*ns,*maxr);
/* Computing MAX */
    d__1 = *drmin, d__2 = *drmax / *hcondbnd;
    *drmin = max(d__1,d__2);
    if (*ns == 1) {
	r__[1] = 1.;
/* Reset! */
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s6Rmod */
/* Subroutine */ int s6rprod_(integer *task, integer *maxr, integer *ns, 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s6Rrow */
/* Subroutine */ int s6rset_(integer *task, integer *maxr, integer *ns, 
	integer *lenr, doublereal *r__, doublereal *w, doublereal *condh)
{
    /* System generated locals */
    integer i__1, i__2;
    doublereal d__1;
    ...
    extern /* Subroutine */ int dload_(integer *, doublereal *, doublereal *, 
	    integer *);
    static doublereal drmin, drmax;
    static integer nzero;

/* ================================================================= */
/* s6Rset  initializes or modifies R, the upper-triangular factor */
/* of the approximate reduced Hessian. */

/* On entry, */
/*   Task = 1. (LOAD)   requests that  R is initialized. */
/*             R is initialized to the identity. */

/*   Task = 2  (MODIFY) the columns of an existing R are scaled by */
/*             by the square roots of the current diagonals. */

/* 12 Jun 2001: First version based on MINOS routine m6rset. */
/* 21 Feb 2004: Current version of s6Rset. */
/* 11 Dec 2014: Task replaces GotR for action request. */
/* ================================================================ */
/* ----------------------------------------------------------------- */
/* ----------------------------------------------------------------- */
    /* Parameter adjustments */
...
/* -------------------------------------------------------------- */
/* Find dRmin and dRmax, and set w = set of scale factors. */
	drmax = abs(r__[1]);
	drmin = drmax;
	ldiag = 1;
	incr = *maxr;
	...
	for (k = 1; k <= i__1; ++k) {
	    ...
	    drmax = max(drmax,diag);
	    drmin = min(drmin,diag);
	    w[k] = 1. / sqrt(diag);
	    ldiag += incr;
	    --incr;
	}
/* Apply column scales to each row of R. */
	...
	*condh = ddiv_(&drmax, &drmin, &overflow);
    }
    return 0;
} /* s6rset_ */
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s6Rsol */
/* Subroutine */ int s6rswap_(integer *maxr, integer *nr, integer *lenr, 
...
/* ================================================================ */
/* s6Rswap  modifies the upper-triangular matrix R to account for a */
...
/* 03 Dec 2000: Converted to row-wise storage. */
/* 17 Jun 2001: Current version of s6Rswap. */
/* ================================================================ */
/* ----------------------------------------------------------------- */
/* ----------------------------------------------------------------- */
    /* Parameter adjustments */
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s6Rswap */
/* Subroutine */ int s6rupdate_(integer *updatetype, integer *maxr, integer *
	lenr, integer *m, integer *n, integer *nbs, integer *nnl, integer *ns,
	 doublereal *u0scal, doublereal *rdxhdx, integer *nea, integer *nloca,
	...
	doublereal *rw, integer *lenrw)
{
    ...
    extern /* Subroutine */ int dscal_(integer *, doublereal *, doublereal *, 
	    ...
	    *, integer *), s2bsol_(integer *, integer *, integer *, 
    ...
    extern /* Subroutine */ int s2bprod_(integer *, doublereal *, integer *, 
	    ...
	    doublereal *, doublereal *, doublereal *, integer *, doublereal *,
	     doublereal *, integer *), s6rbfgs_(integer *, integer *, integer 
	    *, integer *, integer *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *, doublereal *, doublereal *), 
	    s2gather_(integer *, integer *, integer *, doublereal *, 
	    doublereal *, doublereal *);

/* ================================================================= */
/* s6Rupdate computes the effect of the full BFGS update on the */
...
/* 04 Aug 1995: First version (s6Rupdt) based on NPSOL's  npupdt. */
/* 03 Dec 2000: Converted to row-wise storage. */
/* 18 Feb 2001: H stored in product form. */
...
/* ----------------------------------------------------------------- */
    s2gather_(nnl, nbs, &kbs[1], &c_b13, &v[1], &y[1]);
    s2bsol_(&inform__, &c__2, m, &y[1], &w[1], &iw[1], leniw, &rw[1], lenrw);
    if (*ns > 0) {
	s2bprod_(&c__1, &eps0, n, ns, &kbs[*m + 1], nea, nloca, &loca[1], &
		inda[1], &acol[1], &c_b50, &w[1], m, &c_b13, &y[*m + 1], ns);
    }
    s2gather_(nnl, nbs, &kbs[1], &c_b13, &hdx[1], &y1[1]);
    s2bsol_(&inform__, &c__2, m, &y1[1], &w[1], &iw[1], leniw, &rw[1], lenrw);
    if (*ns > 0) {
	s2bprod_(&c__1, &eps0, n, ns, &kbs[*m + 1], nea, nloca, &loca[1], &
		inda[1], &acol[1], &c_b50, &w[1], m, &c_b13, &y1[*m + 1], ns);
    }
    if (*updatetype == 1) {
/* -------------------- */
/* Self-scaled BFGS. */
...
/* ----------------------------------------------------------------- */
    ...
    d__1 = 1. / *rdxhdx;
    s6rbfgs_(&inform__, maxr, ns, nnl, lenr, &told, &tolz, &r__[1], &y1[*m + 
	    1], &y[*m + 1], &d__1, &w[*m + 1]);
    return 0;
} /* s6rupdate_ */

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s6Rupdate */
	...
	doublereal *step, doublereal *told, doublereal *tolz)
{
    ...
    extern /* Subroutine */ int s6rbfgs_(integer *, integer *, integer *, 
	    integer *, integer *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *), s6rprod_(
...
/* 13 Dec 2014: Default iExit = 1 set in s6Rmod. */
/* ================================================================= */
/* ----------------------------------------------------------------- */
/* ----------------------------------------------------------------- */
    /* Parameter adjustments */
...
/* Form   R'*w = -R'Rp in p */
/* 1/sqrt(p'Bp) */
	s6rprod_(&c__1, maxr, ns, lenr, &r__[1], &w[1], &p[1]);
/* Normalize  w  and change its sign. Now w = Rp/norm(Rp) */
	d__1 = -delta1;
	dscal_(ns, &d__1, &w[1], &c__1);
	i__1 = *ns;
...
/* Deal with surplus diagonals of  R. */
    d__1 = -delta1;
    s6rbfgs_(iexit, maxr, ns, ns, lenr, told, tolz, &r__[1], &w[1], &p[1], &
	    d__1, &g[1]);
    return 0;
} /* s6rqn_ */
