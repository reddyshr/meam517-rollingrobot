...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     lPrint, m, n, nb, nlocA, nnL, nnCon, nnJac, neA,
     &     leniw, lenrw, rowType(m), locA(nlocA), indA(neA), iw(leniw)
      double precision
...
!        rowType(i) = 2  for N rows      (free rows)
!
!     Row and column scales Sr and Sc are stored as scales = ( Sc  Sr ).
!
...
!     ==================================================================
     ...
     &     jStart, l, lprScl, lvlScale, mxPass, nclose, npass
      double precision
     ...
     &        // 'Min elem    Max elem       Max col ratio', iw, leniw )
      ...
      if (JustLinear) then
         if (nnJac .ge. n) then
            if (lPrint .gt. 0) then
               call snPRNT( 21,
     &              ' All columns are nonlinear --- '
     &           // ' Row and column scales are all one.', iw, leniw )
            end if
            return
         end if
         iStart = nnCon + 1
...
!     17 Aug 2015: Report file 94, 95 or 96 has the same effect as
!                  Report file 91, 92 or 93, but s2xmat is called
!                  from s4saveB BEFORE UNSCALING.
!     ==================================================================
      integer
     &     i, j, js, k, l, ls, ncol
      double precision
