/* ./src/sn27lu.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int lu1fac_(integer *m, integer *n, integer *nelem, integer *
	...
	*lenr, integer *locc, integer *locr, integer *iploc, integer *iqloc, 
	integer *ipinv, integer *iqinv, integer *cols, integer *markc, 
	integer *markr, doublereal *w, integer *inform__)
{
    /* Format strings */
...
/* Dummy */
    ...
    lu1fad_(m, n, nelem, &lena2, &luparm[1], &parmlu[1], &a[1], &indc[1], &
	    ...
	    iploc[1], &iqloc[1], &ipinv[1], &iqinv[1], &w[1], &cols[1], &
	    markc[1], &markr[1], &lenh, &a[loch], &indc[loch], &indr[loch], &
	    a[lmaxr], inform__, &lenl, &lenu, &minlen, &mersum, &nutri, &
	    nltri, &ndens1, &ndens2, &nrank, &lmax, &umax, &dumax, &dumin, &
	    akmax);
    luparm[16] = nrank;
    luparm[23] = lenl;
    if (*inform__ == 7) {
...
/* Subroutine */ int lu1fad_(integer *m, integer *n, integer *nelem, integer *
	...
	indc, integer *indr, integer *ip, integer *iq, integer *lenc, integer 
	*lenr, integer *locc, integer *locr, integer *iploc, integer *iqloc, 
	integer *ipinv, integer *iqinv, doublereal *w, integer *cols, integer 
	*markc, integer *markr, integer *lenh, doublereal *ha, integer *hj, 
	integer *hk, doublereal *amaxr, integer *inform__, integer *lenl, 
	integer *lenu, integer *minlen, integer *mersum, integer *nutri, 
	integer *nltri, integer *ndens1, integer *ndens2, integer *nrank, 
	doublereal *lmax, doublereal *umax, doublereal *dumax, doublereal *
	dumin, doublereal *akmax)
{
    /* Format strings */
    ...
    static doublereal amax;
    static integer lcol, mark, lenj, imax, jmax, last, leni, hops;
    static logical ltri;
    ...
    static integer nfree, ncold, melim, nelim, lfree;
    static doublereal small;
    static integer mleft, ilast, jlast, minmn, maxmn, nleft, jbest, ibest, 
	    mbest, nrowd, lsave, limit, lpivr, lpivc;
    static doublereal abest;
    static integer kbest, nfill;
    ...
    extern /* Subroutine */ int lu1mrp_(integer *, integer *, integer *, 
	    ...
	    *, integer *, doublereal *, integer *, integer *, integer *, 
	    ...
	    integer *, doublereal *), lu1msp_(integer *, integer *, integer *,
	     integer *, doublereal *, integer *, integer *, integer *, 
	    integer *, doublereal *, integer *, integer *, integer *, integer 
	    *), lu1mxr_(integer *, integer *, integer *, integer *, integer *,
	     integer *, doublereal *, integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *, doublereal *);
    ...
    static cilist io___129 = { 0, 0, 0, fmt_1100, 0 };
    static cilist io___137 = { 0, 0, 0, fmt_1100, 0 };
    static cilist io___138 = { 0, 0, 0, fmt_1200, 0 };
    static cilist io___141 = { 0, 0, 0, fmt_1200, 0 };
...
/* 06 Jun 2013: Call new lu1mxr. */
	mark = 0;
	lu1mxr_(&mark, &c__1, m, m, n, lena, &a[1], &indc[1], &lenc[1], &locc[
		1], &indr[1], &lenr[1], &locr[1], &ip[1], &cols[1], &markc[1],
		 &markr[1], &amaxr[1]);
    }
    if (tcp) {
...
/* Beware: The parts of ip that we need are in indc(ll1:ll */
		++mark;
		lu1mxr_(&mark, &ll1, &ll, m, n, lena, &a[1], &indc[1], &lenc[
			1], &locc[1], &indr[1], &lenr[1], &locr[1], &indc[1], 
			&cols[1], &markc[1], &markr[1], &amaxr[1]);
/* ^^^^  Here are the ip(k1:k2) needed by lu1mx */
	    }
	    if (nelim > 0) {
		i__2 = lu1 + 1;
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine lu1mxc */
/* Subroutine */ int lu1mxr_(integer *mark, integer *k1, integer *k2, integer 
	*m, integer *n, integer *lena, doublereal *a, integer *indc, integer *
	lenc, integer *locc, integer *indr, integer *lenr, integer *locr, 
	integer *p, integer *cols, integer *markc, integer *markr, doublereal 
	*amaxr)
{
    /* System generated locals */
    integer i__1, i__2;
    ...
    /* Local variables */
    static integer i__, j, k, lc, lr, lc1, lc2, lr1, lr2, ncol;
...
/* ------------------------------------------------------------------ */
    ...
    /* Function Body */
    if (*mark == 0) {
/* First call: Find Amaxr(1:m) for origin */
	i__1 = *m;
	...
	for (j = 1; j <= i__1; ++j) {
		...
		i__ = indc[lc];
/* Computing MAX */
		d__2 = amaxr[i__], d__3 = (d__1 = a[lc], abs(d__1));
		amaxr[i__] = max(d__2,d__3);
	    }
	}
    } else {
/* Later calls: Find Amaxr(i) for rows i */
	ncol = 0;
	i__1 = *k2;
	for (k = *k1; k <= i__1; ++k) {
/* Search rows to find which cols are inv */
	    i__ = p[k];
	    markr[i__] = *mark;
/* Mark this row */
	    amaxr[i__] = 0.;
	...
	len, integer *loc)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___366 = { 0, 0, 0, fmt_1000, 0 };
	...
	inform__)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___538 = { 0, 0, 0, fmt_1100, 0 };
	...
	diag, doublereal *vnorm)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___629 = { 0, 0, 0, fmt_1100, 0 };
    static cilist io___630 = { 0, 0, 0, fmt_1200, 0 };
    static cilist io___631 = { 0, 0, 0, fmt_1700, 0 };
    static cilist io___632 = { 0, 0, 0, fmt_1800, 0 };
