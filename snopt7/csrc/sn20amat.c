...
/* Subroutine */ int s2amat_(integer *task, integer *lprint, integer *m, 
	...
	nnobju, integer *iobj, integer *numlc, integer *numliq, integer *nea, 
	integer *nloca, integer *loca, integer *inda, doublereal *acol, 
	doublereal *bl, doublereal *bu, integer *rowtype, integer *iw, 
...
/* Adnsty = 100.0d+0*ne / (m*n) */
	    adnsty = (real) (*nea) * 100.f / (real) (*m) / (real) (*n);
	    s_wsfi(&io___27);
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2Amat */
/* Subroutine */ int s2aprod_(integer *task, doublereal *tolz, integer *nea, 
	integer *nloca, integer *loca, integer *inda, doublereal *acol, 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2Aprod */
/* Subroutine */ int s2bprod_(integer *task, doublereal *tolz, integer *n, 
	integer *lenkbs, integer *kbs, integer *nea, integer *nloca, integer *
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2dInf */
/* Subroutine */ int s2gather_(integer *neg, integer *nbs, integer *kbs, 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2gather */
/* Subroutine */ int s2scatter_(integer *neg, integer *nbs, integer *kbs, 
...
/* Subroutine */ int s2crash_(integer *lcrash, integer *lprint, integer *m, 
	integer *n, integer *nb, integer *nncon, integer *icrash, doublereal *
	tcrash, integer *nea, integer *nloca, integer *loca, integer *inda, 
	doublereal *acol, integer *hpiv, integer *hs, integer *rowtype, 
...
/*        They may be altered later (see the end of Crash). */
	s2aprod_(&c__0, &eps0, nea, nloca, &loca[1], &inda[1], &acol[1], &
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2Mem0 */
/* Subroutine */ int s2mem_(integer *iexit, logical *printmem, integer *
	liwest, integer *lrwest, integer *nextcw, integer *nextiw, integer *
	nextrw, integer *maxcw, integer *maxiw, integer *maxrw, integer *
	lencw, integer *leniw, integer *lenrw, integer *mincw, integer *miniw,
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2Mem */
/* Subroutine */ int s2rca_(logical *feasible, doublereal *featol, integer *
	iobj, integer *minimize, doublereal *wtinf, integer *m, integer *n, 
	integer *nb, integer *leng, integer *neg, integer *nea, integer *
	nloca, integer *loca, integer *inda, doublereal *acol, integer *
	estate, integer *hs, doublereal *bl, doublereal *bu, doublereal *g, 
...
/*        Include the gradient of the linear term. */
	signobj = (doublereal) (*minimize);
	if (*neg > 0) {
	    daxpy_(neg, &signobj, &g[1], &c__1, &rc[1], &c__1);
	}
	if (*iobj > 0) {
	    jobj = *n + *iobj;
	    rc[jobj] += signobj;
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2rcA */
/* Subroutine */ int s2getscales_(integer *lprint, integer *m, integer *n, 
	integer *nb, integer *nnl, integer *nncon, integer *nnjac, integer *
	rowtype, integer *nea, integer *nloca, integer *loca, integer *inda, 
	doublereal *acol, doublereal *scales, doublereal *bl, doublereal *bu, 
	doublereal *rmin, doublereal *rmax, integer *iw, integer *leniw, 
	doublereal *rw, integer *lenrw)
{
    ...
    /* Local variables */
    static integer lvlscale, i__, j, l;
    static doublereal b1, b2, ac, ar;
    static logical justlinear;
    static doublereal big, bnd;
    static char str[80];
    ...
    static doublereal bplus, close1, close2, infbnd, aratio, cratio;
    static integer nclose;
    extern doublereal dnormi_(integer *, doublereal *, integer *);
    static integer lprscl;
    static doublereal scltol, sratio;
...
/* scale tolerance. */
    ...
    aratio = bplus;
    mxpass = 10;
    justlinear = lvlscale == 1;
    dload_(nb, &c_b109, &scales[1], &c__1);
    if (justlinear) {
	if (*nnjac >= *n) {
	    return 0;
	}
...
/* Computing MIN */
		d__1 = amin, d__2 = cmin / scales[j];
		amin = min(d__1,d__2);
/* Computing MAX */
		d__1 = amax, d__2 = cmax / scales[j];
		amax = max(d__1,d__2);
		cratio = cmax / cmin;
...
/*        to adjust both row and column scales. */
	...
	dddiv_(m, &scales[*n + 1], &c__1, &rmax[1], &c__1);
...
/* Computing MAX */
	...
	for (i__ = istart; i__ <= i__1; ++i__) {
	    ...
	    do_fio(&c__1, (char *)&scales[*n + i__], (ftnlen)sizeof(
		    doublereal));
	    e_wsfi();
	    snprnt_(&c__21, str, &iw[1], leniw, (ftnlen)80);
	}
	...
	for (j = jstart; j <= i__1; ++j) {
	    ...
	    do_fio(&c__1, (char *)&scales[j], (ftnlen)sizeof(doublereal));
	    e_wsfi();
	    snprnt_(&c__21, str, &iw[1], leniw, (ftnlen)80);
	}
    }
...
/* Subroutine */ int s2applyscales_(integer *task, integer *m, integer *n, 
	integer *nb, integer *iobj, doublereal *infbnd, doublereal *scaleobj, 
	integer *nea, integer *nloca, integer *loca, integer *inda, 
	doublereal *acol, doublereal *scales, doublereal *bl, doublereal *bu, 
	doublereal *pi, doublereal *x)
{
    /* System generated locals */
    integer i__1, i__2;
...
/*        --------------------------------------------------------------- */
	...
	ddscl_(m, &scales[*n + 1], &c__1, &pi[1], &c__1);
	if (*iobj > 0) {
	    *scaleobj = scales[*n + *iobj];
	}
    } else if (*task == 1) {
/*        --------------------------------------------------------------- */
/*        Unscale everything. */
/*        --------------------------------------------------------------- */
	...
	dddiv_(m, &scales[*n + 1], &c__1, &pi[1], &c__1);
	*scaleobj = 1.;
    }
    return 0;
} /* s2applyscales_ */
...
/* Subroutine */ int s2unpack_(integer *jq, integer *m, integer *n, integer *
	nea, doublereal *colnorm, integer *nloca, integer *loca, integer *
	inda, doublereal *acol, doublereal *y)
{
    /* System generated locals */
    integer i__1;
...
/*     ------------------------------------------------------------------ */
    ...
    *colnorm = 1.;
	...
	for (l = loca[*jq]; l <= i__1; ++l) {
	    ...
	    y[i__] = acol[l];
/* Computing MAX */
	    d__2 = *colnorm, d__3 = (d__1 = y[i__], abs(d__1));
	    *colnorm = max(d__2,d__3);
	}
    } else {
	islack = *jq - *n;
	y[islack] = -1.;
...
/* Subroutine */ int s2dmat_(integer *matfil, integer *n, integer *nb, 
	integer *nea, integer *nloca, doublereal *acol, integer *inda, 
	integer *loca, integer *hs)
{
    /* Format strings */
