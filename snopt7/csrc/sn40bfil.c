...
/* old basis file */
	...
	s4insert_(n, nb, ns, iobj, &hs[1], &bl[1], &bu[1], &x[1], names + 8, &
...
/* Subroutine */ int s4id_(integer *j, integer *n, integer *nb, integer *
	nnames, char *names, char *id, ftnlen names_len, ftnlen id_len)
{
    /* Initialized data */
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4id */
/* Subroutine */ int s4ksave_(integer *minimize, integer *m, integer *n, 
	integer *nb, integer *ns, integer *mbs, integer *itn, integer *ninf, 
	doublereal *sinf, doublereal *f, integer *kbs, integer *hs, 
	doublereal *scales, doublereal *bl, doublereal *bu, doublereal *x, 
...
/* limit on total iterations */
	...
	s4newb_(&c__0, &inewb, minimize, m, n, nb, ns, mbs, itn, ninf, sinf, 
		f, &kbs[1], &hs[1], &scales[1], &bl[1], &bu[1], &x[1], &xbs[1]
		, istate, cw + 8, lencw, &iw[1], leniw, (ftnlen)4, (ftnlen)8);
	if (iback > 0) {
	    s4newb_(&c__0, &iback, minimize, m, n, nb, ns, mbs, itn, ninf, 
		    sinf, f, &kbs[1], &hs[1], &scales[1], &bl[1], &bu[1], &x[
		    1], &xbs[1], istate, cw + 8, lencw, &iw[1], leniw, (
		    ftnlen)4, (ftnlen)8);
	}
    }
    return 0;
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4name */
/* Subroutine */ int s4insert_(integer *n, integer *nb, integer *ns, integer *
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4oldB */
/* Subroutine */ int s4checkhs_(integer *m, integer *maxs, integer *mbs, 
	integer *n, integer *nb, logical *needb, integer *gothes, integer *ns,
...
/* Subroutine */ int s4dump_(integer *idump, integer *m, integer *n, integer *
	nb, integer *nnames, integer *hs, doublereal *x, char *names, char *
	cw, integer *lencw, integer *iw, integer *leniw, ftnlen names_len, 
	ftnlen cw_len)
{
    /* Initialized data */
...
/* Problem name */
    ...
    i__1 = *nb;
    for (j = 1; j <= i__1; ++j) {
	s4id_(&j, n, nb, nnames, names + 8, id, (ftnlen)8, (ftnlen)8);
	k = hs[j] + 1;
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4dump */
/* Subroutine */ int s4newb_(integer *job, integer *inewb, integer *minimize, 
	integer *m, integer *n, integer *nb, integer *ns, integer *mbs, 
	integer *itn, integer *ninf, doublereal *sinf, doublereal *f, integer 
	*kbs, integer *hs, doublereal *scales, doublereal *bl, doublereal *bu,
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4newB */
/* Subroutine */ int s4punch_(integer *ipunch, integer *m, integer *n, 
	integer *nb, integer *nnames, integer *hs, doublereal *bl, doublereal 
	*bu, doublereal *x, char *names, char *cw, integer *lencw, integer *
	iw, integer *leniw, ftnlen names_len, ftnlen cw_len)
{
    /* Initialized data */
...
/* Problem name */
    ...
    for (j = 1; j <= i__1; ++j) {
	jn = j;
	s4id_(&jn, n, nb, nnames, names + 8, colnm, (ftnlen)8, (ftnlen)8);
	k = hs[j];
	if (k == 3) {
...
L300:
		...
		s4id_(&irow, n, nb, nnames, names + 8, name__, (ftnlen)8, (
			ftnlen)8);
		if (k == 2) {
		    k = 0;
		}
		io___169.ciunit = *ipunch;
		s_wsfe(&io___169);
...
/*     Output superbasic slacks. */
	...
	if (hs[j] == 2) {
	    jn = j;
	    s4id_(&jn, n, nb, nnames, names + 8, name__, (ftnlen)8, (ftnlen)8)
		    ;
	    io___171.ciunit = *ipunch;
	    s_wsfe(&io___171);
    ...
    io___172.ciunit = *ipunch;
    s_wsfe(&io___172);
    e_wsfe();
    s_wsfi(&io___174);
    do_fio(&c__1, (char *)&(*ipunch), (ftnlen)sizeof(integer));
    e_wsfi();
    snprnt_(&c__13, str, &iw[1], leniw, (ftnlen)60);
    if (*ipunch != iprint) {
	al__1.aerr = 0;
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4punch */
/* Subroutine */ int s4report_(logical *ondisk, integer *m, integer *n, 
	integer *nb, integer *nnames, integer *nncon0, integer *nncon, 
	integer *nnobj0, integer *nnobj, integer *ns, integer *nea, integer *
	nloca, integer *loca, integer *inda, doublereal *acol, integer *hs, 
	doublereal *scales, doublereal *bl, doublereal *bu, doublereal *gobj, 
	doublereal *pi, doublereal *x, doublereal *fx, char *names, char *
	istate, integer *iw, integer *leniw, ftnlen names_len, ftnlen 
	istate_len)
{
    /* Format strings */
    static char fmt_1000[] = "(\002 XXX Report file requested.  s4report doe"
	    "s nothing.\002)";
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4report */
/* Subroutine */ int s4saveb_(integer *iexit, integer *task, integer *
	minimize, integer *m, integer *n, integer *nb, integer *nkx, integer *
	nncon0, integer *nncon, integer *nnobj0, integer *nnobj, integer *
	nnames, integer *ns, integer *itn, integer *ninf, doublereal *sinf, 
	doublereal *wtinf, doublereal *vimax, integer *iobj, doublereal *
	scaleobj, doublereal *objtrue, doublereal *pnorm1, doublereal *pnorm2,
	 doublereal *pinorm, doublereal *xnorm, integer *nej, integer *nlocj, 
	integer *locj, integer *indj, doublereal *jcol, integer *kx, integer *
	estate, integer *hs, doublereal *scales, doublereal *bl, doublereal *
	...
	ftnlen cw_len)
{
    ...
    extern /* Subroutine */ int s4dump_(integer *, integer *, integer *, 
	    ...
	    *, doublereal *, integer *), s2vmax_(integer *, integer *, 
	    ...
	    , s4stat_(integer *, char *, ftnlen), s4soln_(logical *, integer *
	    , integer *, integer *, integer *, integer *, integer *, integer *
	    , integer *, integer *, integer *, integer *, integer *, integer *
    ...
    static doublereal xnorm1, infbnd;
    extern integer idamax_(integer *, doublereal *, integer *);
    static integer ipunch;
    static char istate[4*3];
    static integer iprint, lprsol;
    extern /* Subroutine */ int snprnt_(integer *, char *, integer *, integer 
	    *, ftnlen), s4punch_(integer *, integer *, integer *, integer *, 
	    integer *, integer *, doublereal *, doublereal *, doublereal *, 
	    char *, char *, integer *, integer *, integer *, ftnlen, ftnlen);
    static integer ireport;
...
/*        Find the maximum bound and dual infeasibilities. */
	s2rca_(&feasible, &tolx, iobj, minimize, wtinf, m, n, nb, nnobj0, 
		nnobj, nej, nlocj, &locj[1], &indj[1], &jcol[1], &estate[1], &
		hs[1], &bl[1], &bu[1], &gobj[1], &pi[1], &rc[1], &x[1]);
...
/*        (Previously, s4soln used the scaled piNorm, but no more.) */
	if (lvlscale > 0) {
	    s2applyscales_(&c__1, m, n, nb, iobj, &infbnd, scaleobj, nej, 
		    ...
		    bu[1], &pi[1], &x[1]);
	    dddiv_(nb, &scales[1], &c__1, &rc[1], &c__1);
/* 27 Dec 2013: We should unscale even for Scale option 1 */
/* if (lvlScale .eq. 2) then */
	    if (*nncon > 0) {
		ddscl_(nncon, &scales[*n + 1], &c__1, &fx[1], &c__1);
	    }
	    if (*nnobj > 0) {
		dddiv_(nnobj, &scales[1], &c__1, &gobj[1], &c__1);
	    }
/* end if */
	    maxx = idamax_(n, &x[1], &c__1);
	    maxp = idamax_(m, &pi[1], &c__1);
...
/*        Output Punch, Dump, Solution and/or Report files. */
	if (ipunch > 0) {
	    s4punch_(&ipunch, m, n, nb, nnames, &hs[1], &bl[1], &bu[1], &x[1],
		     names + 8, cw + 8, lencw, &iw[1], leniw, (ftnlen)8, (
		    ftnlen)8);
	}
	if (idump > 0) {
	    s4dump_(&idump, m, n, nb, nnames, &hs[1], &x[1], names + 8, cw + 
		    8, lencw, &iw[1], leniw, (ftnlen)8, (ftnlen)8);
	}
/* 09 Mar 2004: No longer worry about scaled solution. */
/* piNorm = pNorm1 */
	if (isoln > 0) {
	    s4soln_(&c_true, minimize, m, n, nb, nkx, nnames, nncon0, nncon, 
		    nnobj0, nnobj, ns, iobj, itn, ninf, sinf, objtrue, pinorm,
		     names + 8, nej, nlocj, &locj[1], &indj[1], &jcol[1], &kx[
		    1], &hs[1], &scales[1], &bl[1], &bu[1], &gobj[1], &pi[1], 
...
/* 09 Oct 2000: Export A, B or (B S) to Report file 91, 92 or 93 */
	if (ireport >= 91 && ireport <= 93) {
	    s2xmat_(&ireport, n, nb, nej, nlocj, &locj[1], &indj[1], &jcol[1],
		     &hs[1]);
	} else if (ireport > 0) {
	    s4report_(&c_true, m, n, nb, nnames, nncon0, nncon, nnobj0, nnobj,
		     ns, nej, nlocj, &locj[1], &indj[1], &jcol[1], &hs[1], &
		    scales[1], &bl[1], &bu[1], &gobj[1], &pi[1], &x[1], &fx[1]
...
/* 09 Mar 2004: No longer worry about scaled solution. */
/* piNorm = pNorm1 */
	    s4soln_(&c_false, minimize, m, n, nb, nkx, nnames, nncon0, nncon, 
		    nnobj0, nnobj, ns, iobj, itn, ninf, sinf, objtrue, pinorm,
		     names + 8, nej, nlocj, &locj[1], &indj[1], &jcol[1], &kx[
		    1], &hs[1], &scales[1], &bl[1], &bu[1], &gobj[1], &pi[1], 
		    &rc[1], &x[1], &fx[1], istate, cw + 8, lencw, &iw[1], 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s4saveB */
/* Subroutine */ int s4soln_(logical *ondisk, integer *minimize, integer *m, 
	integer *n, integer *nb, integer *nkx, integer *nnames, integer *
	nncon0, integer *nncon, integer *nnobj0, integer *nnobj, integer *ns, 
	integer *iobj, integer *itn, integer *ninf, doublereal *sinf, 
	doublereal *objall, doublereal *pinorm, char *names, integer *nea, 
	integer *nloca, integer *loca, integer *inda, doublereal *acol, 
	integer *kx, integer *hs, doublereal *scales, doublereal *bl, 
...
/* !    scaled    = lvlScale .gt. 0 */
	...
	s_wsfi(&io___227);
	do_fio(&c__1, mprob, (ftnlen)8);
	do_fio(&c__1, (char *)&(*objall), (ftnlen)sizeof(doublereal));
	e_wsfi();
	if (*ondisk) {
	...
	s_wsfi(&io___234);
	do_fio(&c__1, mobj, (ftnlen)8);
	do_fio(&c__1, objtyp + (*minimize + 1 << 3), (ftnlen)3);
	e_wsfi();
	if (*ondisk) {
	    s1trim_(str, &length, (ftnlen)132);
...
/* ! d1     =   d1 / scale */
/* ! d2     =   d2 / scale */
	s4id_(&jn, n, nb, nnames, names + 8, id, (ftnlen)8, (ftnlen)8);
