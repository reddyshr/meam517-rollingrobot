/* ../snopt7/src/sn25bfac.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int s2bfac_(integer *iexit, integer *typelu, logical *needlu,
	...
	printlevel, integer *lurequest, integer *m, integer *mbs, integer *n, 
	integer *nb, integer *nnl, integer *ns, integer *nswap, integer *nea, 
	integer *nloca, integer *loca, integer *inda, doublereal *acol, 
	integer *kbs, integer *hs, doublereal *bl, doublereal *bu, doublereal 
	*blbs, doublereal *bubs, integer *nrhs0, integer *nrhs, doublereal *
	rhs, doublereal *x, doublereal *xbs, integer *iy, integer *iy1, 
	...
	doublereal *rw, integer *lenrw)
{
    ...
    static integer btask, markr, ntask, ptask, maxiw, maxrw;
    ...
    static integer lprdbg, nslack, matfil, lenalu, inform__, nonlin, maxlui;
...
/*     Umin   is the smallest diagonal of U after last BS factor. */
	...
	if (prtlvl10) {
	    ...
	    do_fio(&c__1, (char *)&(*lurequest), (ftnlen)sizeof(integer));
...
/*           ------------------------------------------------------------ */
	    ...
	    s2blu_(&inform__, &c__2, printlevel, m, n, nb, &nbs, nea, nloca, &
...
/* Report file n */
	    if (matfil > 0) {
		s2dmat_(&matfil, n, nb, nea, nloca, &acol[1], &inda[1], &loca[
			1], &hs[1]);
	    }
	    s2blu_(&inform__, &btask, printlevel, m, n, nb, &nbs, nea, nloca, 
		    &loca[1], &inda[1], &acol[1], &kbs[1], &iw[ip], &rw[lua], 
		    &iw[indc], &iw[indr], &lenalu, &iy[1], &iy1[1], &y[1], &
...
/*              -------------------------------------------------------- */
		...
		bsfac = rw[164] <= utol || rw[164] <= eps2 || singular;
		if (bsfac) {
		    *needlu = TRUE_;
		    ptask = 2;
		    goto L100;
...
/* Then check if any superbasic slacks were made basic. */
		s2sing_(printlevel, mbs, m, n, nb, &y[1], &iw[ip], &iw[iq], &
			bl[1], &bu[1], &hs[1], &kbs[1], &x[1], &iw[1], leniw);
		if (*ns > 0) {
...
/*        --------------------------------------------------------------- */
	s5setx_(&inform__, &c__0, itn, m, n, nb, &nbs, &rowerror, nea, nloca, 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2Bkbs */
/* Subroutine */ int s2bmap_(integer *m, integer *n, integer *nea, integer *
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2newB */
/* Subroutine */ int s2blu_(integer *iexit, integer *task, integer *
	printlevel, integer *m, integer *n, integer *nb, integer *nbs, 
	integer *nea, integer *nloca, integer *loca, integer *inda, 
	doublereal *acol, integer *kbs, integer *ip, doublereal *alu, integer 
	*indc, integer *indr, integer *lenalu, integer *iy, integer *iy1, 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2sb */
/* Subroutine */ int s2sing_(integer *printlevel, integer *mbs, integer *m, 
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s2tols */
/* Subroutine */ int s2trylu_(integer *itn, integer *lurequest0, integer *ns, 
	integer *lurequest, logical *luok, integer *typelu, integer *iw, 
...
/*        LUrequest is used for printing and indicates the reason for */
/*                  the LU request. */
...
/*        Try to find a better basis. */
	*typelu = 2;
    } else if (lumod > 0 && (lumod != 1 || *lurequest != 5)) {
